...
private:
	...
	// ------------------------------------------------------------------------
	void checkCompileErrors(unsigned int shader, std::string type, const char* vertexPath, const char* fragmentPath)
	{
		int success;
		char infoLog[1024];
		if (type != "PROGRAM")
		{
			...
			if (!success)
			{
				...
				std::cout << "Error in files " << vertexPath << " or " << fragmentPath << "\n";
